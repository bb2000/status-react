...
(defn step-back [step editing?]
  (case step
    (:intro :edit) (re-frame/dispatch [:navigate-back])
    (:learn-more :set-snt-amount) (re-frame/dispatch [:set-in [:my-profile/tribute-to-talk :step]
...
(defview set-snt-amount []
  (letsubs [snt-amount [:get-in [:my-profile/tribute-to-talk :snt-amount]]]
    [react/view {:style styles/intro-container}
     ...
     [components.common/button {:button-style styles/intro-button
                                :on-press     #(when-not (string/blank? snt-amount)
    ...
    [react/view {:style styles/intro-container}
     [react/view
      [react/text {:style (assoc styles/description-label :color colors/black
                                 :text-align :left)} (i18n/label :t/personalized-message)
    ...
    [react/view {:style (assoc styles/intro-container :align-items :flex-start)}
     [react/view {:style {:flex-direction :row
      ...
      [react/text {:on-press #(do
       ...
       (i18n/label :t/edit)]]
     [react/text-input {:style (assoc styles/personalized-message-input
                                      :background-color colors/blue-light)
                        :multiline true
                        :editable false
                        :default-value message}]
     [react/text {:style {:font-size 15 :color colors/gray}}
     ...
     [react/touchable-highlight {:on-press #(do
      ...
      [react/view {:style {:flex-direction :row :justify-content :flex-start}}
     ...
     [react/view {:style styles/enabled-note}
      ...
      [react/view {:style {:margin-left 11}}
       [react/text {:style {:font-weight :bold :font-size 15 :text-align-vertical :top}}
        (i18n/label :t/tribute-to-talk-enabled)]
       [react/text {:style {:font-size 15}}
        (i18n/label :t/tribute-to-talk-add-friends)]]]]))

(defview chat-sample []
  [react/view {:style {:border-radius 12
...
(defview learn-more []
  [react/view {:style (assoc styles/intro-container :align-items :flex-start)}
   ...
   [react/view {:style {:flex-direction :row :margin-left 16}}
